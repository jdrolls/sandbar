...
A shared persistent seat publishes only ports 3000 and 3001 to `127.0.0.1`; it does not publish ttyd, the raw desktop-control API, or CDP. Its single `abc`-owned Chromium uses the mode-`0700` `/config/sandbar-browser` profile and binds CDP only to container loopback. The viewer is an unsupported direct-use surface: it is reserved for the approved host-side proxy's WebSocket desktop stream, not a public/tailnet endpoint or a general remote-desktop service. Authentication, human handoff, and browser-control policy are host control-plane responsibilities and are not supplied by those viewer ports.
